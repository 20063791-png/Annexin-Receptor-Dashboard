...
Mean Expression = average expression
across all cells within a cell type.

Percent Positive = percentage of cells
expressing the gene (>0 expression).
...
Showing all cells coloured according
to gene expression intensity.
"""
)
...
Showing only cells with detectable
expression (>0).
"""
)
...
```
    ...
    colB.metric(
        "% Dataset Displayed",
        f"{displayed_cells / adata.n_obs * 100:.1f}%"
    )
